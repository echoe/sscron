...
# Taken from https://raw.githubusercontent.com/cPanelTechs/SysSnapv2/master/sys-snap.pl
# Small modifications by Iris Felshman
...
sub stop_syssnap {
    ...
    if ($pid =~ /[\d+]/) {
        ...
        if ($ps_info =~ /^\s*([0-9]+)\s+root\s+(\/usr\/local\/cpanel\/3rdparty\/bin\/perl\s+\.\/|perl\s+)sys-snap\.pl\s+--start/ ) {
            $running_pid = $1;
        }
...
sub check_status {
    ...
    # this block tries to confirm that there is another intance running that does not match the pid of the newly invoked
        ...
        if( $pids[0] =~ /^\s*([0-9]+)\s+root\s+(\/usr\/local\/cpanel\/3rdparty\/bin\/perl\s+\.\/|perl\s+)sys-snap\.pl\s+[-]{1,2}start/ ) {
            my $tmp_pid = $1;
        ...
        if( $pids[1] =~ /^\s*([0-9]+)\s+root\s+(\/usr\/local\/cpanel\/3rdparty\/bin\/perl\s+\.\/|perl\s+)sys-snap\.pl\s+[-]{1,2}start/ ) {
            my $tmp_pid = $1;
            if($tmp_pid != $current_script) {
                $running_pid=$tmp_pid;
            }
